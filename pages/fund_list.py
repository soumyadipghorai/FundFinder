...
with col2 :
    ...
    num_years = st.slider(
        "Select number of years", step = 1, min_value = 1, 
        max_value = 35 if s_type == "SIP" else 30
    ) 
...
for i in range(len(df)) : 
    obj = CalculateReturns(
        ...
        expense_ratio = df.iloc[i]['expense_ratio'], num_years = num_years, 
        investment_type = s_type
    )
    ...
    total_tax.append(return_before_tax - return_after_tax)
